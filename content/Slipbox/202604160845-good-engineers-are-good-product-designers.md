---
tags:
  - programming
  - product-design
---
>  ... I don’t know any truly good engineers who are also not good at at product design. (https://vickiboykis.com/2026/04/13/mechanical-sympathy/#:~:text=%20I%20don’t%20know%20any%20truly%20good%20engineers%20who%20are%20also%20not%20good%20at%20at%20product%20design.)


> Mechanical sympathy for both developers and end-users means understanding when asyncio is and is not helpful. It means using the right language, the right build system, the right font. It means using the least amount of tooling possible. Allowing for local development. It means reading code inside out rather than top to bottom. [Using uv](https://aleyan.com/blog/2026-why-arent-we-uv-yet/). Removing code where not necessary. Respecting boundaries. (https://vickiboykis.com/2026/04/13/mechanical-sympathy/#:~:text=Mechanical%20sympathy%20for,necessary.%20Respecting%20boundaries.)

